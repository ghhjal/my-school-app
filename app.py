...
# ==========================================
# 👨‍🎓 واجهة الطالب - النسخة الاحترافية النهائية
# ==========================================

if st.session_state.role == "student":
    st.markdown("<h1 style='text-align: center;'>مرحباً بك في لوحة التميز 🌟</h1>", unsafe_allow_html=True)
    ...
    try:
        ...
        if not current_student.empty:
            ...
            # 2. عرض إحصائيات سريعة
            c1, c2 = st.columns(2)
            c1.metric("نقاطي الحالية 🎯", f"{s_data[8]}")
            c2.metric("الصف الدراسي 📚", f"{s_data[2]}")

            # 3. استخدام أسماء تبويبات فريدة (هنا حل مشكلة NameError tab2)
            # لاحظ تسمية المتغيرات student_tab1 وليس tab1
            student_tab1, student_tab2 = st.tabs(["📊 درجاتي المعتمدة", "📜 السلوك والتحفيز"])
            ...
            with student_tab1:
                ...
                if not my_grades.empty:
                    ...
                    st.info("لا توجد درجات مرصودة لهذا الشهر بعد.")

            with student_tab2:
                st.subheader("ملاحظات الأداء السلوكي")
                # تصحيح الخطأ الإملائي البرمجي (إغلاق النص بوضوح)
                st.success("سجلك خالي من الملاحظات السلبية، نحن فخورون بك! ✨")

        else:
            st.warning("⚠️ عذراً، لم يتم العثور على بيانات مرتبطة برقمك الأكاديمي.")

    except Exception as e:
        st.error(f"حدث خطأ أثناء تحميل البيانات: {e}")

    # 4. زر الخروج (مهم جداً للعودة للرئيسية)
    if st.sidebar.button("تسجيل الخروج"):
        st.session_state.role = None
        st.rerun()

    # 5. السطر الأهم لمنع الخطأ: إيقاف البرنامج هنا حتى لا يقرأ كود المعلم بالأسفل
    st.stop()
