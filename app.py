...
if st.session_state.role == "teacher":
    ...
    # ---------------------------------------------------------
    # 👥 التبويب 0: إدارة قاعدة بيانات الطلاب (الربط الديناميكي الكامل)
    # ---------------------------------------------------------
    with menu[0]:
        ...
        if not df_st.empty:
            ...
            # حساب عدد الفصول ديناميكياً من العمود الثالث
            ...
            with st.expander("➕ إضافة طالب جديد (تنسيق ديناميكي مستند إلى الإعدادات)"):
                ...
                with st.form("add_student_v2026_final", clear_on_submit=True):
                    ...
                    # الربط الديناميكي: الخيارات تُقرأ من سلف الإعدادات
                    f_stage = col3.selectbox("🎓 المرحلة", st.session_state.stage_options)
                    f_year = col4.text_input("🗓️ العام الدراسي الحالي", st.session_state.current_year)
                    f_class = col5.selectbox("🏫 الصف الدراسي", st.session_state.class_options)
                    ...
                    if st.form_submit_button("✅ اعتماد وحفظ الطالب"):
                        if f_id and f_name:
                            ...
                            # الخريطة الذكية (Mapping) لضمان مطابقة أعمدة Google Sheets
                            st_map = {
                                "id": f_id.strip(),
                                "name": f_name.strip(),
                                "class": f_class,
                                "year": f_year,
                                "sem": f_stage, # ربط المرحلة بعمود sem كما في هيكلة قاعدة بياناتك
                                "الإيميل": f_mail,
                                "الجوال": f_phone,
                                "النقاط": "0" # تبدأ النقاط بصفر آلياً
                            }
                            
                            # الحفظ الآمن في السحابة
                            if safe_append_row("students", st_map):
                                st.success(f"✅ تم إضافة الطالب {f_name} وتسكينه في {f_class}")
                                st.cache_data.clear() # مسح الكاش لضمان ظهور الطالب في البحث فوراً
                                st.rerun()
                        else:
    ...
    # ---------------------------------------------------------
    # 📢 إدارة التنبيهات والتعميمات (لوحة الإدارة)
    with menu[2]:
        ...
        with st.form("announcement_form_final_v3", clear_on_submit=True):
            ...
            target_list = ["الكل", "الرابع", "الصف الثاني", "الصف الثالث", "الصف الرابع", "الصف الخامس", "الصف السادس", "أولياء الأمور"]
            a_target = c2.selectbox("🎯 الفئة المستهدفة:", target_list)
            
            if st.form_submit_button("📣 نشر وبث التنبيه"):
                if a_title and a_details:
                    # الربط بأسماء الحقول المباشرة لضمان عدم حدوث IndexError
                    ann_data = {
                        "الصف": a_target,
                        "العنوان": a_title,
                    ...
                    if safe_append_row("exams", ann_data):
                        st.success(f"✅ تم النشر لـ {a_target}")
                        st.cache_data.clear()
                        st.rerun()

        st.divider()
        st.markdown("#### 📜 سجل التعميمات (بث الواتساب)")
        ...
        if not df_ann.empty:
            for idx, row in df_ann.iloc[::-1].iterrows():
                with st.container(border=True):
                    ...
                    with col_info:
                        st.write(f"📢 **{row.get('العنوان')}** | 🎯 الموجه لـ: {row.get('الصف')}")
                        st.caption(f"📝 {row.get('الرابط')}")
                    
                    with col_actions:
                        # 1. زر إرسال للواتساب (تمت إعادته)
                        whatsapp_msg = urllib.parse.quote(f"📢 *تنبيه منصة زياد*\n📌 *{row.get('العنوان')}*\n📝 {row.get('الرابط')}")
                        st.link_button("👥 بث واتساب", f"https://api.whatsapp.com/send?text={whatsapp_msg}", use_container_width=True)
...
if st.session_state.role == "student":
    ...
    if not my_info.empty:
        ...
        s_class = str(s_data.get('class', 'غير محدد')).strip() # تحديد صف الطالب للفلترة
        s_points = int(pd.to_numeric(s_data.get('النقاط', 0), errors='coerce') or 0)
        
        # 🎨 تنسيق التباين العالي للجوال والأوسمة الأفقية
        st.markdown(f"""
            <style>
        ...
        # 🚨 1. عرض "التنبيهات العاجلة" في الشاشة الرئيسية (مفلترة بالصف)
        if not df_ann.empty:
            if df_ann.shape[1] >= 5:
                # فلترة التنبيهات العاجلة الموجهة لصف الطالب المسجل أو للكل
                urgent_news = df_ann[(df_ann.iloc[:, 4] == 'نعم') & (df_ann.iloc[:, 0].isin(['الكل', s_class]))]
                if not urgent_news.empty:
                    latest = urgent_news.tail(1).iloc[0]
                    st.markdown(f"""<div class="urgent-msg">📢 تنبيه عاجل لـ {s_class}: {latest.iloc[1]} <br> <small style="font-weight:normal;">{latest.iloc[3]}</small></div>""", unsafe_allow_html=True)
