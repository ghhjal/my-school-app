...
if st.session_state.role == "teacher":
    ...
    # ==========================================
    # ⚙️ تبويب: الإعدادات والأدوات الإدارية الشاملة (إصدار 2026 المطور)
    # ==========================================
    with menu[3]:
        st.subheader("⚙️ مركز التحكم وإدارة النظام")
    
        # --- ⚖️ 1. إعدادات توزيع الدرجات (الحفظ الدائم في جدول settings) ---
        st.markdown("#### ⚖️ إعدادات توزيع الدرجات")
        with st.expander("تعديل الحدود العليا للدرجات (توزيع الوزارة السنوي)", expanded=True):
            col_g1, col_g2 = st.columns(2)
            
            # التأكد من وجود القيم في الذاكرة لتجنب الانهيار
            max_t_val = st.session_state.get('max_tasks', 60)
            max_q_val = st.session_state.get('max_quiz', 40)
            
            new_max_t = col_g1.number_input("الحد الأعلى للمشاركة والمهام", 1, 100, max_t_val)
            new_max_q = col_g2.number_input("الحد الأعلى للاختبار القصير", 1, 100, max_q_val)
            
            if st.button("💾 اعتماد وحفظ التوزيع الجديد نهائياً"):
                try:
                    ws_s = sh.worksheet("settings")
                    # تحديث القيم في الإكسل بناءً على السطور في صورتك (2 للمشاركة، 3 للاختبار)
                    ws_s.update_cell(2, 2, new_max_t)
                    ws_s.update_cell(3, 2, new_max_q)
                    
                    # تحديث الذاكرة الحالية لضمان التطبيق الفوري
                    ...
                    st.error(f"⚠️ فشل الحفظ الدائم في شيت settings: {e}")
    
        st.divider()
    
        # --- 🔐 2. إدارة الحساب وكلمات المرور (حل خطأ IndexError) ---
        with st.expander("🔐 إدارة كلمات المرور وتأمين الحساب", expanded=False):
            df_u = fetch_safe("users")
            if not df_u.empty:
                # جلب المستخدمين الفعليين (z1, Ziyad1) من جدولك
                user_list = df_u['username'].tolist()
                user_to_fix = st.selectbox("اختر المستخدم للتعديل:", user_list)
                new_pass = st.text_input("🔑 كلمة المرور الجديدة", type="password")
                
                if st.button("تحديث وتشفير كلمة المرور"):
                    if new_pass:
                        u_hash = hashlib.sha256(str.encode(new_pass)).hexdigest()
                        # إيجاد السطر الصحيح وحل مشكلة IndexError
                        row_idx = df_u[df_u['username'] == user_to_fix].index[0] + 2
                        sh.worksheet("users").update_cell(row_idx, 2, u_hash)
                        st.success(f"✅ تم تحديث وتشفير كلمة مرور {user_to_fix} بنجاح")
                    else:
                        st.warning("يرجى إدخال كلمة مرور جديدة.")
            else:
                st.error("⚠️ لم يتم العثور على مستخدمين في قاعدة البيانات.")
    
        st.divider()
    
        # --- 📥 3. تحميل القوالب الفارغة (Excel Templates) ---
        ...
        # --- 📤 4. أداة رفع الملفات للمنصة (Uploader) ---
        st.markdown("#### 📤 رفع البيانات من ملف Excel")
        st.info("تأكد من مطابقة أعمدة الملف للقوالب المحملة أعلاه لضمان الاستقرار.")
        ...
        if uploaded_file is not None:
            if st.button(f"🚀 بدء عملية الرفع إلى {upload_type}"):
                try:
                    ...
                    st.error(f"⚠️ خطأ: تأكد من مطابقة الأعمدة للقالب. (التفاصيل: {e})")
# ==========================================
