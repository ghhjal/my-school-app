...
if st.session_state.role == "student":
    ...
    # وضعنا كل شيء داخل try لتجنب أي توقف مفاجئ
    try:
        # جلب البيانات (تأكد أن اسم الدالة fetch_safe صحيح عندك)
        df_students = fetch_safe("students")
        
        # البحث عن سطر الطالب
        student_row = df_students[df_students.iloc[:, 0].astype(str).str.strip() == str(st.session_state.sid)]
        ...
        if not student_row.empty:
            ...
            # إنشاء تبويبات بأسماء فريدة جداً لا تتكرر في كود المعلم
            # هذا يحل خطأ NameError: tab2 نهائياً
            t_grades, t_behavior = st.tabs(["📊 درجاتي", "🌟 نقاطي وسلوكي"])

            with t_grades:
                st.write("### سجل الدرجات")
                df_g = fetch_safe("grades")
                my_g = df_g[df_g.iloc[:, 0].astype(str).str.strip() == str(st.session_state.sid)]
                st.dataframe(my_g, use_container_width=True)

            with t_behavior:
                st.write("### السجل السلوكي")
                st.info(f"رصيد نقاطك الحالي هو: {s_data[8]}")
                # تأكد من إغلاق علامة الاقتباس هنا لتجنب SyntaxError
                st.write("استمر في الاجتهاد للوصول إلى قائمة الشرف! 🏆")

        else:
            st.error("لم يتم العثور على بياناتك. يرجى التأكد من الرقم الأكاديمي.")

    except Exception as e:
        st.error(f"حدث خطأ تقني: {e}")

    # 🛑 أهم سطرين في الكود كله لحل مشكلتك:
    if st.button("تسجيل الخروج"):
        st.session_state.role = None
        st.rerun()
        
    # هذا السطر يمنع البرنامج من النزول لأسفل وقراءة كود المعلم (tab2)
    st.stop() 

# ==========================================
# 🛑 نهاية قسم الطالب (لن يرى الطالب ما بعد هذا السطر)
# ==========================================
