...
# --- 2. دالة PDF مصلحة (تجنب UnicodeError) ---
def generate_safe_pdf(info, grades, logs):
    ...
    # نستخدم نصوصاً إنجليزية في الـ PDF لتفادي انهيار البرنامج بسبب الترميز العربي
    ...
    pdf.ln(10)
    pdf.cell(200, 10, txt="Behavior Records:", ln=True)
    for _, row in logs.iterrows():
        # تنظيف النص من الرموز التعبيرية التي تسبب الخطأ في الصورة السادسة
        clean_note = "".join(c for c in row['note'] if ord(c) < 128)
        pdf.cell(200, 8, txt=f"- {row['date']}: {clean_note}", ln=True)
...
# --- 4. واجهات البرنامج ---
    ...
    if st.session_state.role == 'admin':
        ...
        if not g.empty:
            ...
            st.columns(3)[0].metric("فترة 1", g.iloc[0]['p1'])
            st.columns(3)[1].metric("فترة 2", g.iloc[0]['p2'])
            st.columns(3)[2].metric("مهام", g.iloc[0]['perf'])
