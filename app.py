            ...
            if not df_st.empty:
                ...
                if st.button("تأكيد الحذف"):
                    if to_del:
                        ...
                        st.cache_data.clear(); st.success("تم الحذف"); st.rerun()
        with col_add:
                ...
                with col_b:
                    ...
                    if not df_bh.empty:
                        ...
                        for _, row in my_bh.iterrows():
                            ...
                            # شرط الاختفاء: يختفي إذا كان محدثاً في جوجل أو تم ضغطه في الجلسة الحالية
                            ...
                            if not is_r:
                                if st.button(f"🙏 شكراً أستاذي زياد", key=f"thx_{r_id}"):
                                    # 1. اختفاء فوري من الواجهة (Optimistic Update)
                                    st.session_state.confirmed_rows.add(r_id)
                                    
                                    # 2. التحديث في الخلفية مع معالجة الخطأ بصمت
                                    try:
                                        sh.worksheet("behavior").update_cell(r_id, 5, "✅ تمت القراءة")
                                        st.cache_data.clear() # مسح الذاكرة المؤقتة للطلب القادم
                                        st.balloons()
                                        time.sleep(0.5)
                                        st.rerun()
                                    except:
                                        # حتى لو فشل الاتصال، سيبقى الزر مختفياً عند الطالب بسببconfirmed_rows
                                        pass
