...
# ==========================================
    ...
    with menu[2]:
        ...
        # --- 1️⃣ قسم نشر تنبيه جديد (كود الأستاذ زياد المكتمل) ---
        with st.expander("🚀 نشر إعلان أو موعد اختبار جديد", expanded=True):
            with st.form("flexible_announcement_form", clear_on_submit=True):
                c1, c2 = st.columns([2, 1])
                ann_title = c1.text_input("📝 عنوان التنبيه")
                ...
                # خيار التحكم في الظهور على الشاشة الرئيسية (للمرونة الكاملة)
                show_on_home = st.checkbox("🌟 عرض هذا الإعلان بشكل بارز في الشاشة الرئيسية؟")
                ...
                if st.form_submit_button("📣 نشر الآن للمنصة"):
                    if ann_title:
                        try:
                            # تحديد ما إذا كان الإعلان عاجلاً للشاشة الرئيسية
                            is_urgent = "نعم" if show_on_home else "لا"
                            
                            # إضافة السطر لجدول الإعلانات (5 أعمدة الآن)
                            sh.worksheet("exams").append_row([
                                ann_target, 
                                ann_title, 
                                str(ann_date), 
                            ...
                            # ✨ الحل التقني: حفظ موقع التبويب وتحديث البيانات
                            st.session_state.active_tab = 2 
                            st.cache_data.clear()
                            st.rerun()
                            
                        except Exception as e:
                            st.error(f"⚠️ حدث خطأ أثناء الاتصال بقاعدة البيانات: {e}")
                    else:
                        st.warning("⚠️ يرجى كتابة عنوان للتنبيه أولاً.")
        
        st.divider()
    
        # --- 2️⃣ إدارة التنبيهات السابقة وإرسال المجموعات ---
        st.markdown("##### 📜 سجل التنبيهات وإرسال المجموعات")
        df_ex = fetch_safe("exams")
        ...
        if not df_ex.empty:
            for index, row in df_ex.iloc[::-1].iterrows():
                with st.container(border=True):
                    # عرض معلومات الإعلان
                    status_home = "🏠 يظهر في الرئيسية" if len(row) > 4 and row.iloc[4] == "نعم" else "📱 تنبيه عادي"
                    st.markdown(f"**[{row.iloc[0]}]** - **{row.iloc[1]}** | `{status_home}`")
                    st.caption(f"📅 التاريخ: {row.iloc[2]}")
                    
                    # تجهيز رسالة الواتساب للمجموعات بتنسيق احترافي
                    msg_text = (
                    ...
                    if c_del.button("🗑️ حذف", key=f"del_ann_{index}", use_container_width=True):
                        try:
                            sh.worksheet("exams").delete_rows(int(index) + 2)
                            st.session_state.active_tab = 2 # البقاء في تبويب التنبيهات
                            st.cache_data.clear()
                            st.rerun()
                        except:
                            st.error("فشل في حذف التنبيه")
        else:
