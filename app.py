...
if st.session_state.role == "teacher":
    ...
    # ---------------------------------------------------------
    # ---------------------------------------------------------
    # 📢 التبويب 2: إدارة التنبيهات والتعميمات العامة (إصدار 2026)
    # ---------------------------------------------------------
    with menu[2]:
        ...
        with st.form("admin_announcement_final_v2026", clear_on_submit=True):
            ...
            c1, c2 = st.columns(2)
            # ميزة العرض في الشاشة الرئيسية كبانر أحمر عاجل
            ...
            if st.form_submit_button("📣 نشر وبث التنبيه الآن"):
                if a_title and a_details:
                    # تحضير البيانات لضمان المطابقة مع أعمدة قوقل شيت (exams)
                    ann_data = {
                        "الصف": a_target,
                        "عاجل": "نعم" if is_urgent else "لا",
                        "العنوان": a_title,
                        "التاريخ": str(datetime.date.today()),
                        "الرابط": a_details # يتم حفظ التفاصيل في حقل "الرابط" لضمان التوافق
                    }
        ...
        if not df_ann.empty:
            # عرض التنبيهات من الأحدث للأقدم لسهولة المتابعة
            for idx, row in df_ann.iloc[::-1].iterrows():
                with st.container(border=True):
                    col_txt, col_btn = st.columns([3, 1])
                    
                    with col_txt:
                        # تمييز التنبيه العاجل بصرياً في السجل
                    ...
                    with col_btn:
                        # ✅ ميزة بث الواتساب الاحترافي للمجموعات
                        ...
                        if st.button("🗑️ حذف التنبيه", key=f"del_ann_{idx}", use_container_width=True):
                            try:
                                # حذف السطر البرمجي (المؤشر + 2 لتعويض الترويسة وبدء الفهرس من 0)
