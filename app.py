...
# ==========================================
# ==========================================
# 👨‍🏫 واجهة المعلم الاحترافية (بدون قائمة جانبية)
# ==========================================
if st.session_state.role == "teacher":
    # 1. العنوان العلوي العريض (تصميم متجاوب مع الجوال)
    st.markdown(f"""
        <div style="background: linear-gradient(135deg, #1e3a8a, #3b82f6); padding: 20px; margin: -1rem -1rem 1rem -1rem; border-bottom: 5px solid #f59e0b; text-align: center;">
            <h2 style="color: white; margin: 0; font-family: 'Cairo', sans-serif; font-size: 1.4rem;">
                👨‍🏫 لوحة تحكم المعلم
            </h2>
            <p style="color: #e0e7ff; margin: 5px 0 0 0; font-size: 0.9rem;">أهلاً بك أ. زياد في منصتك المدرسية</p>
        </div>
    """, unsafe_allow_html=True)

    # 2. نظام التبويبات العلوية (بديل للقائمة الجانبية المزعجة)
    # هذا النظام يضمن اختفاء الشريط الجانبي الأبيض تماماً
    ...
    with t_manage:
        st.markdown('<h3 style="text-align:right; color:#1e3a8a;">👥 إدارة سجل الطلاب</h3>', unsafe_allow_html=True)
        # --- ضع كود عرض جدول الطلاب وتأسيس طالب جديد هنا ---
        # مثال:
        df_st = fetch_safe("students")
    ...
    with t_grades:
        ...
        # --- ضع كود استمارة رصد الدرجات هنا ---

    with t_behavior:
        st.markdown('<h3 style="text-align:right; color:#1e3a8a;">🎭 رصد السلوك والتميز</h3>', unsafe_allow_html=True)
        # --- ضع كود إضافة نقاط السلوك هنا ---

    with t_exams:
        st.markdown('<h3 style="text-align:right; color:#1e3a8a;">📢 إضافة إعلانات واختبارات</h3>', unsafe_allow_html=True)
        # --- ضع كود إضافة الاختبارات الجديدة هنا ---

    with t_logout:
        st.warning("هل أنت متأكد من رغبتك في تسجيل الخروج؟")
        if st.button("نعم، تسجيل الخروج الآن", use_container_width=True):
            st.session_state.role = None
            st.rerun()

    # نصيحة: لإخفاء الشريط الجانبي تماماً من الجوال، ابحث عن زر (X) في القائمة الجانبية وأغلقه مرة واحدة.
