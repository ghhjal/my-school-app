...
# --- القسم الثالث: رصد السلوك (إصدار الجوال والحاسوب المطور) ---
    ...
    if b_name:
        # جلب البيانات: العمود G للإيميل (6) والعمود H للجوال (7)
        student_info = df_st[df_st.iloc[:, 1] == b_name].iloc[0]
        s_email = student_info[6] # العمود G
        s_phone = str(student_info[7]).split('.')[0] # العمود H مع تنظيف الرقم
        ...
        with st.container(border=True):
            with st.form("behavior_v15", clear_on_submit=True):
                ...
                btn_mail = col2.form_submit_button("📧 حفظ وإيميل")
                btn_wa = col3.form_submit_button("💬 حفظ وواتساب")

                if btn_save or btn_mail or btn_wa:
                    if b_note:
                        # 1. حفظ الملاحظة في شيت السلوك
                        sh.worksheet("behavior").append_row([b_name, str(b_date), b_type, b_note])
                        
                        # 2. تحديث إجمالي النقاط في شيت الطلاب (العمود I هو رقم 8)
                        try:
                            ws_st = sh.worksheet("students")
                            cell = ws_st.find(b_name)
                            p_map = {"🌟 متميز (+10)": 10, "✅ إيجابي (+5)": 5, "⚠️ تنبيه (0)": 0, "❌ سلبي (-5)": -5, "🚫 مخالفة (-10)": -10}
                            # جلب النقاط الحالية من العمود I (رقم 8)
                            current_val = ws_st.cell(cell.row, 9).value
                            current_p = int(current_val) if current_val and str(current_val).isdigit() else 0
                            new_p = current_p + p_map.get(b_type, 0)
                            ws_st.update_cell(cell.row, 9, str(new_p))
                        except Exception as e:
                            st.error(f"فشل تحديث النقاط: {e}")
                        ...
                        # إرسال الإيميل
                        if btn_mail and s_email and s_email != "None":
                        ...
                        # إرسال الواتساب
                        if btn_wa and s_phone and s_phone != "None":
                        ...
                        st.success(f"✅ تم الحفظ وتحديث النقاط الطالب: {b_name}")
                        time.sleep(1)
                        st.rerun()
                    else:
                        st.error("⚠️ يرجى كتابة نص الملاحظة")

        # عرض تاريخ السلوك للطالب المختار
        df_b = fetch_safe("behavior")
        if not df_b.empty:
            st_history = df_b[df_b.iloc[:, 0] == b_name]
            if not st_history.empty:
                st.markdown("### 🕒 السجل التاريخي للطالب")
