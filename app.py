...
if st.session_state.role == "teacher":
    ...
    # ==========================================
    # 📢 تبويب: التواصل والتنبيهات (نسخة التحكم المرن)
    # ==========================================
    with menu[2]:
        st.subheader("📢 مركز إدارة التنبيهات")
        
        with st.expander("🚀 نشر تنبيه جديد مع خيارات العرض", expanded=True):
            with st.form("flexible_announcement_form", clear_on_submit=True):
                ...
                ann_details = st.text_area("📄 تفاصيل الإعلان")
                
                # 💡 الميزة الجديدة: خيار التحكم في الظهور على الشاشة الرئيسية
                show_on_home = st.checkbox("🌟 هل تريد عرض هذا الإعلان بشكل بارز في الشاشة الرئيسية؟")
                
                ann_date = st.date_input("🗓️ تاريخ النشر", datetime.date.today())
                
                if st.form_submit_button("📣 نشر للمنصة"):
                    if ann_title:
                        # إضافة العمود الخامس (نعم/لا) للتحكم في الظهور
                        ...
                        sh.worksheet("exams").append_row([
                            ...
                            str(ann_date), 
                            ann_details,
                            is_urgent  # القيمة الجديدة التي تتحكم في الرئيسية
                        ])
