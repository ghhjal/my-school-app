        ...
        if not df_bh.empty:
            ...
            if not my_bh.empty:
                ...
                for index, row in my_bh.iterrows():
                    ...
                    # زر الشكر التفاعلي - تحديث الحالة في جوجل شيت
                    if st.button(f"🙏 شكراً أستاذي", key=f"thx_btn_{index}"):
                        # 1. تحديث البيانات في الشيت (بافتراض وجود عمود اسمه 'الحالة')
                        # ملاحظة: سنستخدم الـ index لتحديد الصف الصحيح
                        try:
                            df_bh.at[index, 'الحالة'] = "✅ قرأها وشكرك"
                            # هنا تضع كود الرفع للشيت (مثال: conn.update)
