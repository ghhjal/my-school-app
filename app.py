...
if st.session_state.role == "teacher":
    ...
    if menu == "👥 إدارة الطلاب":
        ...
        # استعادة تبويبات التسجيل والبحث
        ...
        with tab_view:
            ...
            if not df_st.empty:
                ...
                # --- الحذف الشامل الحقيقي (حل مشكلة السلوك) ---
                st.divider()
                st.subheader("🗑️ حذف طالب (شامل لجميع الجداول)")
                ...
                if st.button("تأكيد الحذف الشامل"):
                    if del_name:
                        with st.spinner("جاري تنظيف كافة السجلات..."):
                            # 1. حذف السلوك بالاسم (الطريقة الأضمن لعدم بقاء بيانات)
                            try:
                                ws_bh = sh.worksheet("behavior")
                                all_bh = ws_bh.get_all_records()
                                # إعادة بناء الجدول بدون الطالب المختار
                                clean_bh = [r for r in all_bh if str(list(r.values())[0]).strip() != del_name.strip()]
                                ws_bh.clear()
                                if all_bh: ws_bh.append_row(list(all_bh[0].keys()))
                                if clean_bh: ws_bh.append_rows([list(r.values()) for r in clean_bh])
                            except: pass
                            
                            # 2. حذف الدرجات (grades)
                            try:
                                ws_gr = sh.worksheet("grades"); c_gr = ws_gr.find(del_name.strip())
                                ws_gr.delete_rows(c_gr.row)
                            except: pass
                            
                            # 3. حذف الطالب من القائمة الرئيسية
                            ws_st = sh.worksheet("students"); c_st = ws_st.find(del_name.strip())
                            ws_st.delete_rows(c_st.row)
                            
                            st.success(f"🗑️ تم حذف {del_name} وكافة بياناته بنجاح"); time.sleep(1); st.rerun()
        ...
        if not df_all.empty:
            ...
            with t_beh:
                with st.form("behavior_add_form"):
