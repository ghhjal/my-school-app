...
def fetch_safe(worksheet_name):
    """جلب البيانات بشكل آمن وتجنب أخطاء الأعمدة"""
    if not sh: return pd.DataFrame()
    try:
        ws = sh.worksheet(worksheet_name)
        data = ws.get_all_values()
        if len(data) < 1: return pd.DataFrame()
        # تنظيف البيانات من المسافات الزائدة في العناوين
...
# ==========================================
# 2. التصميم (CSS) - الحفاظ الكامل على التصميم
# ==========================================
st.markdown("""
    ...
    with tab2:
        with st.form("te_login"):
            ...
            if st.form_submit_button("تسجيل الدخول"):
                df = fetch_safe("users")
                if not df.empty:
                    row = df[df.iloc[:, 0] == u] # العمود 0 هو username
                    if not row.empty and hashlib.sha256(str.encode(p)).hexdigest() == row.iloc[0, 1]: # العمود 1 هو hash
...
if st.session_state.role == "teacher":
    ...
    with tabs[1]:
        ...
        if not df_st.empty:
            with st.form("grades_entry_smart"):
                ...
                c1, c2, c3 = st.columns(3)
                p1 = c1.number_input("📝 المهام (P1)", 0.0, 100.0, step=0.5)
                p2 = c2.number_input("📄 الاختبار (P2)", 0.0, 100.0, step=0.5)
                total_score = p1 + p2
                st.info(f"المجموع الحالي: {total_score}")
                ...
                if st.form_submit_button("💾 حفظ واعتماد الدرجة"):
                    ...
                    if cell:
                        ws_g.update(range_name=f"B{cell.row}:F{cell.row}", values=[data_row[1:]])
                    else:
    ...
    with tabs[3]:
        ...
        if not df_st.empty:
            ...
            if b_name:
                ...
                s_email, s_phone = st_row.iloc[6], str(st_row.iloc[7])
                with st.container(border=True):
                    c1, c2 = st.columns(2)
                    ...
                    if col1.button("💾 حفظ فقط"):
                        ...
                        c_cell = ws_st.find(b_name)
                        if c_cell:
                            p_map = {"🌟 متميز (+10)": 10, "✅ إيجابي (+5)": 5, "⚠️ تنبيه (0)": 0, "❌ سلبي (-5)": -5, "🚫 مخالفة (-10)": -10}
                            current_p = int(ws_st.cell(c_cell.row, 9).value or 0)
                            ws_st.update_cell(c_cell.row, 9, current_p + p_map.get(b_type, 0))
    ...
    with tabs[5]:
        ...
        if st.button("🔴 تصفير نقاط جميع الطلاب"):
            ...
            if len(all_vals) > 1:
                # تحديث العمود التاسع (I) بالكامل ليكون 0
                cell_list = ws.range(f'I2:I{len(all_vals)}')
...
# ==========================================
    ...
    try:
        ...
        st.error("حدث خطأ في جلب بياناتك"); st.stop()

    st.markdown(f"<h2 style='text-align:center;'>مرحباً {s_name} | نقاطك: {pts}</h2>", unsafe_allow_html=True)
    t1, t2, t3 = st.tabs(["📢 تنبيهات", "📊 درجات", "🏆 ترتيب"])
    
    with t1:
        df_ex = fetch_safe("exams")
        if not df_ex.empty:
            # فلترة حسب الصف أو "الكل"
            my_ex = df_ex[(df_ex.iloc[:, 0] == s_row.iloc[2]) | (df_ex.iloc[:, 0] == "الكل")]
            for _, r in my_ex.iloc[::-1].iterrows():
                st.info(f"📢 {r.iloc[1]} | 📅 {r.iloc[2]}")
    ...
    with t2:
        ...
        if not my_g.empty:
            r = my_g.iloc[0]
            st.metric("المجموع النهائي", r.iloc[3])
            st.write(f"ملاحظة المعلم: {r.iloc[5]}")
