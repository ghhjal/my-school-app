        ...
        if not all_students:
           ...
           # --- 2. تبويب السلوك (التوافق مع الأعمدة واليوم التلقائي) ---
            with t2:
                ...
                try:
                    # محاولة جلب البيانات من قوقل شيت
                    ...
                    if len(b_vals) > 1:
                        ...
                        for i, row in enumerate(reversed(b_vals[1:])):
                            real_idx = len(b_vals) - i # تحديد مكان الصف الأصلي للحذف
                            ci, cd = st.columns([5, 1])
                            with ci:
                                # عرض البيانات بالترتيب المنسق: الاسم | التاريخ (اليوم) | النوع | الوصف
                                # نستخدم try-except داخلية لضمان عدم توقف العرض إذا فقدت قيمة واحدة
                                try:
                                    n, d, t, note, dy = row[0], row[1], row[2], row[3], row[4]
                                    st.warning(f"👤 **{n}** | 🗓️ {d} ({dy}) | {t} | 🎭 {note}")
                                except:
                                    st.info("سجل قيد المزامنة...")
                            with cd:
                                if st.button("🗑️", key=f"del_final_{real_idx}"):
                    ...
                    # بدلاً من الرسالة الحمراء، سيظهر هذا النص الصغير
                    st.write("🔄 جاري مزامنة السجل مع قوقل شيت...")
