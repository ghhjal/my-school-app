...
st.markdown("""
    ...
    /* تحسين شكل البطاقات والأزرار للجوال */
    .stButton>button {
        width: 100%;
        height: 60px;
        border-radius: 15px;
    ...
    /* إخفاء القوائم الافتراضية لزيادة المساحة */
...
def get_db():
    try:
        ...
        # استخدام الـ Key الخاص بك كما هو في كودك
...
# ==========================================
# 🚪 شاشة الدخول الاحترافية (المطورة للجوال)
# ==========================================
if st.session_state.role is None:
    ...
    # استخدام التبويبات (Tabs) بشكل أنيق للفصل بين المعلم والطالب
...
if st.session_state.role == "teacher":
    st.sidebar.markdown(f"### 👨‍🏫 أهلاً أستاذ زياد")
    ...
    if menu == "👥 إدارة الطلاب":
        ...
        st.markdown('<div style="background:linear-gradient(90deg,#F59E0B,#D97706);padding:20px;border-radius:15px;color:white;text-align:center;"><h1>🎭 رصد السلوك</h1></div>', unsafe_allow_html=True)
        df_st = fetch_safe("students")
        search_term = st.text_input("🔍 ابحث عن اسم الطالب")
        filtered_names = [n for n in df_st.iloc[:, 1].tolist() if search_term in n]
        ...
        st.markdown('<div style="background:linear-gradient(90deg,#4F46E5,#3B82F6);padding:20px;border-radius:15px;color:white;text-align:center;"><h1>📢 شاشة الاختبارات</h1></div>', unsafe_allow_html=True)
...
# ==========================================
    ...
    st.markdown(f"""<div style="background:#1e3a8a; padding:15px; color:white; text-align:center; border-radius:10px;"><h3>🎯 الطالب: {s_name}</h3></div>""", unsafe_allow_html=True)
    
    t_ex, t_grade, t_beh = st.tabs(["📢 التنبيهات", "📊 درجاتي", "🎭 السلوك"])
    ...
    with t_grade:
        try:
            ...
            c1, c2, c3 = st.columns(3)
            c1.metric("المشاركة", g_row[1])
            c2.metric("الواجبات", g_row[2])
            c3.metric("الاختبارات", g_row[3])
    ...
    with t_beh:
        ...
        if not df_beh.empty:
            ...
            st.dataframe(f_beh, use_container_width=True)
