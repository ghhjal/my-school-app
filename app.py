...
# ==========================================
# القاعدة الأولى: st.set_page_config يجب أن يكون أول أمر Streamlit
...
def get_gspread_client():
    try:
        ...
        st.error("⚠️ فشل الاتصال بقاعدة البيانات. تحقق من الإعدادات.")
        return None

sh = get_gspread_client()

# --- تحميل الإعدادات الدائمة (لضمان بقاء التوزيع حتى بعد الخروج) ---
...
def fetch_safe(worksheet_name):
    try:
        ...
        # معالجة المعرف (ID) لضمان استقرار البحث
        if not df.empty:
...
# ==========================================
# 👨‍🏫 5. واجهة المعلم (المكتملة والمستقرة)
# ==========================================
if st.session_state.role == "teacher":
    menu = st.tabs(["👥 الطلاب", "📊 التقييم", "📢 التواصل", "⚙️ الإعدادات", "🚗 خروج"])

    with menu[0]: # تبويب الطلاب: إحصائيات + بحث ذكي + إدارة
        ...
        if not df_st.empty:
            ...
            with st.expander("🗑️ منطقة الحذف الآمن"):
                del_id = st.selectbox("اختر الرقم للحذف:", [""] + df_st.iloc[:, 0].tolist())
                if st.button("🚨 حذف الطالب نهائياً") and del_id:
                ...
                with col_g:
                    ...
                    with st.form("grade_form"):
                        ...
                        if st.form_submit_button("💾 حفظ"):
                            ...
                            else: st.error("⚠️ الدرجة تتجاوز الحد المسموح به في الإعدادات.")
                with col_b:
                    ...
                    with st.form("beh_form", clear_on_submit=True):
                        ...
                        if st.form_submit_button("💾 حفظ وإرسال واتساب"):
                            ...
                            msg = urllib.parse.quote(f"📢 تنبيه من منصة زياد\nالطالب: {s_info.iloc[1]}\nالسلوك: {b_type}\nالملاحظة: {b_msg}")
                            st.markdown(f'<a href="https://api.whatsapp.com/send?phone={s_info.iloc[6]}&text={msg}" target="_blank">📲 اضغط هنا لإرسال الرسالة</a>', unsafe_allow_html=True)
                            st.success("تم الحفظ"); st.cache_data.clear()

    with menu[2]: # تبويب التواصل: التنبيهات مع تثبيت الصفحة
        ...
        with st.form("ann_form", clear_on_submit=True):
            ...
            if st.form_submit_button("📣 نشر الآن"):
                ...
                st.success("تم النشر"); st.session_state.active_tab = 2; st.cache_data.clear(); st.rerun()

    with menu[3]: # تبويب الإعدادات: حفظ دائم + رفع إكسل + باسوورد
        st.subheader("⚙️ أدوات التحكم المتقدمة")
        with st.expander("⚖️ توزيع الدرجات السنوي (حفظ دائم)"):
            c1, c2 = st.columns(2)
        ...
        with st.expander("🔐 تغيير كلمة المرور"):
            df_u = fetch_safe("users")
            user_fix = st.selectbox("اختر المستخدم:", df_u['username'].tolist() if not df_u.empty else [])
...
if st.session_state.role == "student":
    ...
    with t_lead: # لوحة العشرة الأوائل
        top_10 = df_s.sort_values(by='النقاط', ascending=False).head(10)
