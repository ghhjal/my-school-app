...
# --- التبويب الجديد: البحث الذكي الشامل ---
    with t_search:
        ...
        # 2. حقل البحث (يبحث في كل شيء)
        search_query = st.text_input("🔎 ادخل (الاسم، الرقم الأكاديمي، أو رقم الجوال) للبحث عنه:", placeholder="اكتب هنا...")

        if search_query:
            # منطق البحث: تحويل كل القيم لنصوص والبحث في جميع الأعمدة
            mask = df_st.apply(lambda row: row.astype(str).str.contains(search_query, case=False).any(), axis=1)
            results = df_st[mask]

            if not results.empty:
                st.success(f"✅ تم العثور على ({len(results)}) نتيجة")
                
                # 3. اختيار وتنسيق الأعمدة المطلوبة فقط (حسب طلبك)
                # id (0), name (1), class (2), الجوال (7), النقاط (8)
                # نقوم بترتيبها وتسميتها لتظهر بشكل احترافي
                ...
                # ميزة إضافية: إحصائية سريعة لنتائج البحث
                st.info(f"💡 نصيحة: يمكنك النقر على اسم العمود في الجدول أعلاه لترتيب النتائج.")
            else:
                st.warning("❌ لا توجد نتائج مطابقة لبحثك، تأكد من البيانات المدخلة.")
        else:
            st.info("👆 يرجى كتابة أي معلومة عن الطالب في الحقل أعلاه لبدء البحث.")
