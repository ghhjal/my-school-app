        ...
        if not df_bh.empty:
            ...
            if not my_bh.empty:
                ...
                for index, row in my_bh.iterrows():
                    ...
                    # كود الزر داخل شاشة الطالب
if st.button(f"🙏 شكراً أستاذي", key=f"thx_{index}"):
    try:
        # 1. تحديد الصف الصحيح في الشيت (index + 2 لأن الشيت يبدأ من 1 وهناك عنوان)
        # نفترض أن العمود رقم 5 هو المخصص للإشعارات
        # يمكنك تغيير الرقم 5 حسب ترتيب الأعمدة عندك
        sh_behavior.update_cell(index + 2, 5, "✅ تمت القراءة") 
        
        st.balloons() # احتفال للطالب عند نجاح الإرسال
        st.toast("وصل شكرك للأستاذ زياد في لوحة التحكم! 🌸")
    except Exception as e:
        # في حال وجود أي مشكلة في الاتصال، يظهر تأكيد محلي للطالب
