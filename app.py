...
# --- وظيفة تحديث حالة القراءة (الحل المضمون) ---
def mark_as_read(s_name, dt, note):
    try:
...
if st.session_state.role == "teacher":
    ...
    if menu == "👥 إدارة الطلاب":
        ...
        # (إضافة/حذف الطلاب - كما هي في الكود الأصلي)
...
# --- 4. واجهة الطالب (الحل النهائي لزر الشكر) ---
            ...
            for idx, row in my_bh.iterrows():
                ...
                st.markdown(f"""
                    <div style="background-color: {bg}; padding: 15px; border-radius: 12px; border-right: 8px solid {'#1B5E20' if is_read else '#E65100'}; margin-bottom: 10px;">
                        <b>{bh_type}</b> | <small>📅 {dt}</small><br>💬 {note}<br>
                        <small>الحالة: <b>{status}</b></small>
                    </div>
                """, unsafe_allow_html=True)
                
                # الزر يظهر فقط إذا لم تُقرأ الملاحظة
                if not is_read:
                    # استخدام المفتاح الفريد لمنع تكرار الأزرار
                    btn_id = f"btn_{s_name}_{idx}"
                    if st.button(f"🙏 شكراً أستاذي زياد (تأكيد القراءة)", key=btn_id):
                        with st.spinner("جاري التحديث..."):
                            success = mark_as_read(s_name, dt, note)
                            if success:
                                st.balloons()
                                st.success("شكراً لك! تم إبلاغ المعلم بقراءتك للملاحظة.")
                                time.sleep(1)
                                st.rerun()
                            else:
