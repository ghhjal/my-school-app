...
def get_connection():
    return sqlite3.connect('school_v10_final.db', check_same_thread=False)
...
# --- 4. واجهات النظام ---
    ...
    if st.session_state.role == 'admin':
        ...
        if menu == "👥 إدارة الطلاب":
            ...
            st.button("➕ إضافة طالب جديد (تفريغ الحقول)", on_click=clear_student_fields)
            
            with st.form("st_form"):
                c1, c2 = st.columns(2)
                fid = c1.number_input("الرقم الأكاديمي", min_value=1, key="id_key")
                fname = c2.text_input("اسم الطالب بالكامل", key="name_key")
                flevel = c1.selectbox("المرحلة الدراسية", ["ابتدائي", "متوسط", "ثانوي"], key="level_key")
                fclass = c2.text_input("الصف (مثلاً: 1/أ)", key="class_key")
                
                if st.form_submit_button("حفظ بيانات الطالب"):
                    if fname.strip() == "": st.error("يرجى إدخال اسم الطالب")
                    else:
            ...
            st.divider()
            st.subheader("📋 قائمة الطلاب (المرحلة والصف)")
            df_s = pd.read_sql_query("SELECT * FROM students", conn)
            for _, r in df_s.iterrows():
                with st.container(border=True):
                    col1, col2, col3 = st.columns([3, 2, 1])
                    col1.write(f"👤 **{r['name']}** (ID: {r['id']})")
                    col2.write(f"🏫 **{r['level']}** - الصف: {r['grade_class']}")
                    if col3.button("🗑️ حذف", key=f"del_st_{r['id']}"):
        ...
        # القسم الثاني: رصد الدرجات (تعديل وحذف)
            ...
            if not st_df.empty:
                target_st = st.selectbox("اختر الطالب", st_df['name'])
                tid = int(st_df[st_df['name'] == target_st]['id'].values[0])
                
                # جلب الدرجات الحالية للتعديل الفوري
                ...
                with st.form("gr_form"):
                    st.write(f"🖊️ تعديل درجات: **{target_st}**")
                    g1, g2, g3 = st.columns(3)
                    p1 = g1.number_input("فترة 1", 0.0, 20.0, value=v1)
                    p2 = g2.number_input("فترة 2", 0.0, 20.0, value=v2)
                ...
                if not cur_g.empty:
                    st.divider()
                    if st.button(f"🗑️ حذف كافة درجات {target_st}"):
        ...
        # القسم الثالث: سجل السلوك
            ...
            st_df = pd.read_sql_query("SELECT id, name FROM students", conn)
            if not st_df.empty:
                target_st = st.selectbox("الطالب", st_df['name'])
                ...
                for _, ln in logs.iterrows():
                    with st.container(border=True):
                        ...
                        c_a.write(f"📅 {ln['date']} ({ln['day']}) | {ln['type']}: {ln['note']}")
                        if c_b.button("🗑️", key=f"db_{ln['id']}"):
                            c.execute("DELETE FROM behavior WHERE id=?", (ln['id'],))
                            conn.commit()
                            st.rerun()

    # واجهة الطالب (إصلاح مشكلة الأخطاء البرمجية)
        ...
        g_res = pd.read_sql_query("SELECT * FROM grades WHERE student_id=?", conn, params=(sid,))
        if not g_res.empty:
            st.divider()
            st.write("### 📊 الدرجات الدراسية")
            c1, c2, c3 = st.columns(3)
            c1.metric("فترة 1", g_res.iloc[0]['p1'])
            c2.metric("فترة 2", g_res.iloc[0]['p2'])
            c3.metric("مشاركة", g_res.iloc[0]['perf'])
            
        st.divider()
        st.write("### 📅 سجل السلوك")
        b_res = pd.read_sql_query("SELECT date AS التاريخ, day AS اليوم, type AS النوع, note AS الملاحظة FROM behavior WHERE student_id=?", conn, params=(sid,))
        if not b_res.empty: st.table(b_res)
        else: st.info("سجل السلوك نظيف")
