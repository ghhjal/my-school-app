...
# ==========================================
# واجهة الطالب (تم تعديلها تقنياً لتعمل فوراً)
# ==========================================

# تم تغييرها إلى if مستقلة لضمان عدم تعطل الكود بسبب التعليقات السابقة
if st.session_state.role == "student":
    ...
    try:
        ...
        if not student_data.empty:
            ...
            # عرض الترحيب
            st.success(f"مرحباً بك يا {s_name} | الفصل: {s_class}")
            st.metric("رصيد نقاطك", f"{s_points} نقطة")
            
            # --- إصلاح مشكلة التبويبات (NameError) ---
            # يجب تعريف tab1 و tab2 قبل استخدامهما
            tab1, tab2 = st.tabs(["📊 درجاتي", "🗓️ جدول الاختبارات"])
            
            with tab1:
                # تصفية درجات هذا الطالب فقط
            ...
            with tab2:
                # عرض جدول الاختبارات لفصل الطالب
                my_exams = df_ex[df_ex.iloc[:, 1] == s_class]
                if not my_exams.empty:
                    st.table(my_exams)
                else:
                    st.info("لا توجد اختبارات مجدولة لفصلك.")
                    
        else:
            st.error("⚠️ لم يتم العثور على بياناتك في سجل الطلاب.")
    except Exception as e:
