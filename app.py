...
with t2:
    df_b = fetch_safe("behavior")
    if not df_b.empty:
        # تصفية الملاحظات الخاصة بهذا الطالب فقط
        student_notes = df_b[df_b.iloc[:, 0] == s_name]
        
        for i, row in student_notes.iterrows():
            # قراءة الحالة الحالية من العمود الخامس
            current_status = row.iloc[4] if len(row) > 4 else "لم تتم القراءة"
            
            with st.container(border=True):
                st.write(f"📅 {row.iloc[1]} | {row.iloc[2]}")
                st.info(row.iloc[3])
                
                # الشرط: إذا كانت الحالة "لم تتم القراءة" يظهر الزر
                if current_status == "لم تتم القراءة":
                    if st.button(f"❤️ شكراً أستاذي", key=f"thanks_{i}"):
                        ws_b = sh.worksheet("behavior")
                        # تحديث الحالة في جوجل شيت فوراً
                        # نبحث عن السطر الصحيح (رقم السطر في الشيت هو الفهرس + 2)
                        ws_b.update_cell(i + 2, 5, "تمت القراءة")
                        st.success("تم إرسال الشكر للأستاذ")
                        time.sleep(0.5)
                        st.rerun() # إعادة تحميل الصفحة لإخفاء الزر وتغيير الحالة
                else:
                    # إذا كانت الحالة "تمت القراءة" يختفي الزر ويظهر هذا النص
