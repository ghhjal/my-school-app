...
with st.sidebar:
    ...
    st.divider()
    # إضافة إشراف الأستاذ زياد المعمري في مكان بارز [المطلوب]
    st.markdown("---")
    st.markdown("### ✍️ إشراف:")
    st.info("**الأستاذ زياد المعمري**")

# --- 3. واجهة المعلم (ثابتة بدون تغيير في الحقول) ---
if st.session_state.role == "teacher":
    ...
    if menu == "👥 إدارة الطلاب":
        ...
        with t_view:
            ...
            st.divider()
            del_target = st.selectbox("🗑️ اختر طالب للحذف النهائي من كافة السجلات", [""] + df_st["الاسم"].tolist())
